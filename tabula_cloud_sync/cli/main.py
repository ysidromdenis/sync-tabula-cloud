...
def init(
    ...
    if not force and not detector.is_new_project():
        if not click.confirm(
            "Ya existe configuración de Tabula Cloud Sync. ¿Continuar?"
        ):
...
def info(output: str):
    ...
    project_info = {
        ...
        "configuration": {
            ...
            "services": [str(s) for s in detector.get_existing_services()],
            "is_configured": not detector.is_new_project(),
        },
    ...
    if save_json_file(output, project_info):
        ...
        click.echo(f"❌ Error exportando información")
        sys.exit(1)
